<template>
    ...
    <div class="form">
        <div class="flex">
            <div class="left-items">
                <h2>Editing Employee {{ this.form.first_name }}</h2>
            </div>
        ...
        <div>
            ...
            <img class="thumbnail block" :src="this.hostname + '/images_api/images/people/leadership/' + this.form.leadership_picture">
            <input id="leadership" type="file" ref="fileInput1" @change="addLeadership()" class="form-control" />
        </div>
    
        <div>
            <label>Personal Picture</label>
            <img class="thumbnail block" :src="this.hostname + '/images_api/images/people/personal/' + this.form.personal_picture">
            <input id="leadership" type="file" ref="fileInput2" @change="addPersonal()" class="form-control" />
    ...
    export default {
        ...
        created() {
            let self = this;
            axios.get(this.hostname + '/get_people_employee/' + this.$route.params.select).then(function (response) {
                console.log(response);
                self.form.first_name = response.data[0].first_name;
                self.form.last_name = response.data[0].last_name;
                self.form.leadership_picture = response.data[0].leadership_picture;
                self.form.personal_picture = response.data[0].personal_picture;
                self.form.description = response.data[0].description;
                self.form.certifications = response.data[0].certifications;
                self.form.role = response.data[0].role;
                self.form.active = response.data[0].active;
        ...
        methods: {
            submit() {
                axios
                    .post(
                        this.hostname + "/editemployee/" + this.$route.params.select,
                        this.form
                    )
