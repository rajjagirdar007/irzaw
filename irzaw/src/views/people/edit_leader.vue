...
<div class="form">
    <div class="flex">
        <div class="left-items">
            <h2>Editing {{ this.form.first_name }}</h2>
        </div>
...
export default {
	...
	console.log(this.$route.params.select);
        let self = this;
        axios.get(this.hostname + '/get_people/' + this.$route.params.select).then(function (response) {
    ...
    methods: {
        submit() {
            axios
                .post(
                    this.hostname + "/editleader/" + this.$route.params.select,
                    this.form
                )
